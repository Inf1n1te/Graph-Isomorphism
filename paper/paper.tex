...
\item $\forall a, b \in V_1, \{a, b\} \in E_1$ if and only if $\{f(a), f(b)\} \in E_2$.
\end{compactenum}
...
\begin{compactenum}[\upshape(a)]
\item $f$ is a graph isomorphism.
\item $\forall v \in V_1$: $color(v)$ = $color (f(v)$. 
\end{compactenum}
...
\end{theorem}
\begin{proof}
$\Leftarrow$:  $\exists$ graph isomorphism $h: H_1 \to H_2$. If $v \in V_{H1}$ has color 0: we define $g(v) = h(v)$.
...
Define a module $M_i \subseteq V_{G1}: \{v \in V_{G1}$ $|$ corresponding vertex in $H_1$ has color $i \}$. $\forall a, b \in V_{G1}$ such that $a,b \in M_i$ for some $i$: $\{a,b\} \in E_{G1} \iff \{c,d\} \in E_{H1}$. $c,d$ are the corresponding vertices of resp. $a,b$. $\{c,d\} \in E_{H1} \iff \{h(c),h(d)\} \in E_{H2} \iff \{g(a),g(b)\} \in E_{G2}$, because $g(a)$ and $g(b)$ are elements of the corresponding (false) twinset of resp. $c$ and $d$ and all those elements have the same neighbours.


%$\forall a, b \in V_{G1}$ such that $a \in M_i$ and $b \in M_j$ for some $i,j: i\neq j$:
